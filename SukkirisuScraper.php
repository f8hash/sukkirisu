...
require_once('Interface/ScraperInterface.php');

class SukkirisuScraper implements ScraperInterface
{
    private $result;
    ...
    public function __construct(SiteInterface $site, ParserInterface $parser)
    {
        $elements = $parser->find($site->html(), $this->selector);

        foreach ($this->ranking_list($elements) as $rank => $res) {
    ...
    public function result($birthMonth = '1月'): string
    {
